...
export const type = getCompletions([store.class_, store.valuetype, store.object, "pinned", "typedref", store.void_, `${store.value} ${store.class_}`, `${store.native} ${store.int}`, `${store.native} ${store.unsigned} ${store.int}`, `${store.native} ${store.uint}`]);
